...
class DriveMatrix:
    ...
    def compute_r_theta_field(self, start: int, end: int) -> dict[int, tuple[float, float]]:
        """
    ...
    def compute_sequences(self, max_n: int) -> dict[str, dict[int, int]]:
        ...
        self.computation_results["sequences"] = {
            "fibonacci": dict(list(self.sequence_cache["fibonacci"].items())[:max_n + 1]),
            "lucas": dict(list(self.sequence_cache["lucas"].items())[:max_n + 1])
        }
        self.state = MatrixState.COMPLETE
